...
const Request = require('./../plugins/fbs/request');
const Response = require('./../plugins/fbs/response');
...
it('Build XML message', function() {
    return setup().then(function(app) {
        const req = new Request(app.services.bus, config.fbs);
        let xml = req.buildXML('990xxx2.00');
...
it('Parse XML error message', function(done) {
    ...
    const xml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><ns2:sip xmlns:ns2="http://axiell.com/Schema/sip.xsd"><error>Required field: LOGIN_PASSWORD must be set!</error></ns2:sip>';
    const res = new Response(xml, 'AO');
