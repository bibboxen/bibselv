...
export function fakeHandleAction(action, context) {
    function getRandomHandinBook() {
        const bookToReturn =
            context.loanedBooks.get[
                Math.floor(Math.random() * context.loanedBooks.get.length)
            ];
        return bookToReturn;
    }
    ...
    function handleBookHandin(book) {
        ...
        justHandedInMaterials = justHandedInMaterials.filter(
            (fakeBook) => book.id !== fakeBook.id
        );
        justHandedInMaterials.push(book);
        context.justHandedInBooks.set(justHandedInMaterials);
       
        let loanedMaterials = context.loanedBooks.get;
        loanedMaterials = loanedMaterials.filter(
            (fakeBook) => book.id !== fakeBook.id
        );
        context.loanedBooks.set(loanedMaterials);
    }

    function getRandomBook() {
        const bookToReturn =
            fakeBooks[Math.floor(Math.random() * fakeBooks.length)];
    ...
    function handleBookLoan(book) {
        ...
        justLoanedMaterials = justLoanedMaterials.filter(
            (fakeBook) => book.id !== fakeBook.id
        );
