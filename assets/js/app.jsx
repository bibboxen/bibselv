...
function App({ uniqueId, socket }) {
  ...
  function setupTokenRefresh() {
    ...
    const nextRefresh = Math.max(
      expire - Math.floor(Date.now() / 1000) - refreshTime,
      30
    );
  ...
  function loadTranslations(languageCode) {
    ...
    // Default to english.
    const selectedLanguageCode = supportedLanguageCodes.includes(languageCode)
      ? languageCode
      : "en";

    import("../../public/lang/" + selectedLanguageCode + "-comp.json").then(
      (data) => {
