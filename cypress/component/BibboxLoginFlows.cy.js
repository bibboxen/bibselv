...
describe('Login flows', () => {
    it('loginScanUsernamePassword is rendered and responds to barcode interaction', () => {
        ...
        cy.mount(
            <IntlProvider locale="da" messages={messages}>
                <Bibbox
                    ...
                    machineStateInput={ { step: 'loginScanUsernamePassword', flow: 'status' }}
                    errorMessage={null}
        ...
        cy.get('[data-cy="sub-header"]')
            .should('have.text', 'Brug scanneren til at scanne stregkoden på dit bibliotekskort');
        ...
        cy.get('[data-cy="page-header"]')
            .should('have.text', 'Login');
        cy.get('[data-cy="sub-header"]')
            .should('have.text', 'Indtast din adgangskode');
        cy.get('input')
            .should('exist');
    });
