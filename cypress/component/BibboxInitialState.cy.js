...
describe("Initial", () => {
  it("Initial is rendered and responds to mouse interaction", () => {
    ...
    cy.get('[data-cy="barcodes"]').find(".col-md-3").should("have.length", 3);

    // Frontend calls actionhandler enterflow with checkInItems on click
    cy.get('[data-cy="bubble-buttons"]').find(".bubble.checkinitems").click();
    cy.get("@actionHandlerStub").should("be.calledWith", "enterFlow", {
      flow: "checkInItems",
    });

    // Frontend calls actionhandler enterflow with status on click
    cy.get('[data-cy="bubble-buttons"]').find(".bubble.status").click();
    cy.get("@actionHandlerStub").should("be.calledWith", "enterFlow", {
      flow: "status",
    });

    // Frontend calls actionhandler enterflow with checkOutItems on click
    cy.get('[data-cy="bubble-buttons"]').find(".bubble.checkoutitems").click();
  ...
  it("Initial responds to barcode scanner", () => {
    ...
    cy.get("@actionHandlerStub").should("not.be.calledWith", "checkInItem", {
